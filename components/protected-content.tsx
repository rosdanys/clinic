    ...
    queryFn: async () => {
      ...
      const lowStockCount = inventoryItems
        ? inventoryItems.filter(
            (item: any) =>
              (item.initial_quantity + item.entries - item.exits) <
              item.min_stock
          ).length
        : 0;
              ...
              <Link key={item.href} href={item.href} passHref>
                ...
                >
                  ...
                  {sidebarOpen && badgeVal > 0 && (
                    <Badge
                      className={`h-5 min-w-5 shrink-0 rounded-full px-1.5 text-[10px] font-bold flex items-center justify-center border-0 ${getBadgeColor(
                        item.badgeKey
                      )}`}
                    >
                      {badgeVal}
